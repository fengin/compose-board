...
> **操作粒度**：Profile 仍负责整组启用/停用；Profile 已启用后，组内服务与固定服务共用同一套单服务操作规则（启停 / 重启 / 重建 / 升级）。
> **原因**：同一 profile 下的服务通常有功能关联（如 fdfs-tracker + fdfs-storage），单独启用无意义。

| 功能 | 优先级 | 说明 |
|------|--------|------|
| Profile 分组展示 | 🔴 高 | 可选服务按 profile 分组，标注配置启用态（`enabled` / `disabled`） |
| 启用 Profile | 🔴 高 | 整组启用：`docker-compose --profile <name> up -d` |
| 停用 Profile | 🔴 高 | 整组停用：stop + rm 该 profile 下所有服务 |
| 停用确认 | 🔴 高 | 弹出确认框，列出将被停止的服务 |
| Profile 已启用后的服务操作 | 🔴 高 | Profile 已启用后，组内服务与固定服务使用同一套单服务操作规则；未启用时，未部署服务不显示 `[启动]` |

> **未启用的 Profile 服务**（有 profile 且当前 profile 为 disabled）不显示单服务启动按钮，只显示 `⭕ 未部署` 状态 + profile 标签。
> **已启用的 Profile 服务** 与固定服务一致，按实际运行状态展示单服务操作按钮。
...
| 可选服务建模 | optional 不再作为 category，而是由 profile 维度表达和分组；profile 状态只区分 `enabled` / `disabled` 两种配置态 |
